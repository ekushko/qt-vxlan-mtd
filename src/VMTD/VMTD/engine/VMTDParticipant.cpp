...
namespace VMTDLib
{
    ...
    void VMTDParticipant::buildCommands()
    {
        ...
        // TODO 1.1 (KEA) Шлюзовые подсети
        // Добавить в настройки внешние подсети и VLAN ID, а также имена узлов, которые являются шлюзами в эти подсети
        // Нужно для того, чтобы можно было маршрутизировать данные через сеть с подвижными целями и тестировать её
        // удалить отсюда
        vlans += ", 17, 18";
        // удалить досюда
